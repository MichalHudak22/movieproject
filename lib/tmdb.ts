...
===================================================================================== */
async function fetchMultiplePages(endpoint: string, totalResults: number, language = 'en-US') {
...
export async function getTrendingMovies(limit = 30) {
  const results = await fetchMultiplePages('/trending/movie/week', limit);
  return { results };
}

// Popular movies – 30
export async function getPopularMovies(limit = 30) {
  const results = await fetchMultiplePages('/movie/popular', limit);
  return { results };
}

// Top rated movies – 30
export async function getTopRatedMovies(limit = 30) {
  const results = await fetchMultiplePages('/movie/top_rated', limit);
  return { results };
}
...
export async function getTrendingTV(limit = 30) {
  const results = await fetchMultiplePages('/trending/tv/week', limit);
  return { results };
}

// Popular TV – 30
export async function getPopularTV(limit = 30) {
  const results = await fetchMultiplePages('/tv/popular', limit);
  return { results };
}

// Top rated TV – 30
export async function getTopRatedTV(limit = 30) {
  const results = await fetchMultiplePages('/tv/top_rated', limit);
  return { results };
}
...
export async function getRisingPeople() {
  const res = await fetch(`${BASE_URL}/person/popular?api_key=${API_KEY}&page=2`);
